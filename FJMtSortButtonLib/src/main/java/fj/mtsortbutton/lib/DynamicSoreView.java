...
public class DynamicSoreView<T> extends LinearLayout {
    ...
    public DynamicSoreView(Context context, AttributeSet attrs) {
        ...
        if (typedArray != null) {
            //选中点
            RadioSelect = typedArray.getResourceId(R.styleable.DynamicSoreView_SoreRadioSelect, R.mipmap.radio_select);
            //未选中点
            RadioUnselected = typedArray.getResourceId(R.styleable.DynamicSoreView_SoreRadioUnselected, R.mipmap.radio_unselected);
            //圆点间距
    ...
    public DynamicSoreView setNumColumns(GridView gridView){
        gridView.setNumColumns(number);//设置每行GridView个数
        return this;
    }
    /**
     * 设置初始化
