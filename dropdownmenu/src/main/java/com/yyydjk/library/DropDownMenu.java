...
public class DropDownMenu extends LinearLayout {
    ...
    private void addTab(@NonNull List<String> tabTexts, int i) {
        ...
        tab.setLayoutParams(new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT, 1.0f));
        tab.setTextColor(textUnselectedColor);
        tab.setCompoundDrawablesWithIntrinsicBounds(null, null, getResources().getDrawable(menuUnselectedIcon), null);
        tab.setText(tabTexts.get(i));
        tab.setPadding(dpTpPx(50), dpTpPx(12), dpTpPx(50), dpTpPx(12));
        //添加点击事件
        ...
        if (i < tabTexts.size() - 1) {
            ...
            layoutParams.setMargins(0,25,0,25);//4个参数按顺序分别是左上右下
        ...
        for (int i = 0; i < tabMenuView.getChildCount(); i = i + 2) {
            if (target == tabMenuView.getChildAt(i)) {
                if (current_tab_position == i) {
                    ...
                    ((TextView) tabMenuView.getChildAt(current_tab_position)).setTextAppearance(getContext(), R.style.text_style);
